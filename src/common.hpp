...
const float PROJECTILE_SIZE = 10 * WORK_SCALE_FACTOR;
...
const float ROW_BUFF_UI_WIDTH = 20.0f * UI_SCALE;
const float ROW_BUFF_UI_HEIGHT = 20.0f * UI_SCALE;
const float ROW_BUFF_SPACING = 12 + ROW_BUFF_UI_WIDTH;
const int BUFF_NUM = (static_cast<int>(std::floor(
						  (WINDOW_WIDTH_PX - UI_MARGIN_X * 2 - NUCLEUS_UI_WIDTH - HEALTH_BAR_WIDTH - WEAPON_PILL_UI_WIDTH - UI_SPACING) / ROW_BUFF_SPACING)) /
					  2) *
					 2;
const vec2 BUFF_START_POS = {
	-(BUFF_NUM / 2) / 2 * ROW_BUFF_SPACING,
...
const float POPUP_DURATION = 4500;
